...
public class Programa extends laBaseVisitor<Tipos>{
    ...
    public Tipos visitTermo_logico(laParser.Termo_logicoContext ctx){
        ...
        for(int i = 0; i < ctx.outrosf.size(); i++){ // Para cada termo do lado direito,
            ...
            if(opts.tiposCompativeis(fator_logico, tipoLadoEsquerdo) == Tipos.Erro) // Verifica se os tipos de cada lado são compatíveis.
                erro = true;
            else
                tipoLadoEsquerdo = opts.tiposCompativeis(fator_logico, tipoLadoEsquerdo); // Se não houve erro, obtém se o novo tipo, por exemplo float + int = float.
        }
    ...
    public Tipos visitExp_relacional(laParser.Exp_relacionalContext ctx){
        ...
        for(int i = 0; i < ctx.outrasexp.size(); i++){ // Para cada termo do lado direito,
            ...
            if(opts.tiposCompativeis(ladoEsquerdo, exp_aritmetica) == Tipos.Erro) // Verifica se os tipos de cada lado são compatíveis.
                erro = true;
            else
                ladoEsquerdo = opts.tiposCompativeis(ladoEsquerdo, exp_aritmetica); // Se não houve erro, obtém se o novo tipo, por exemplo float + int = float.
        }
    ...
    public Tipos visitExp_aritmetica(laParser.Exp_aritmeticaContext ctx){
        ...
        for(int i = 0; i < ctx.outrostermos.size(); i++){ // Para cada termo do lado direito,
            ...
            if(opts.tiposCompativeis(termo, ladoEsquerdo) == Tipos.Erro) // Verifica se os tipos de cada lado são compatíveis.
                erro = true;
            else
                ladoEsquerdo = opts.tiposCompativeis(termo, ladoEsquerdo); // Se não houve erro, obtém se o novo tipo, por exemplo float + int = float.
        }
    ...
    public Tipos visitTermo(laParser.TermoContext ctx){
        ...
        for(int i = 0; i < ctx.outrosfatores.size(); i++){ // Para cada termo do lado direito,
            ...
            if(opts.tiposCompativeis(fator, ladoEsquerdo) == Tipos.Erro) // Verifica se os tipos de cada lado são compatíveis.
                erro = true;
            else
                ladoEsquerdo = opts.tiposCompativeis(fator, ladoEsquerdo); // Se não houve erro, obtém se o novo tipo, por exemplo float + int = float.
        }
    ...
    public Tipos visitFator(laParser.FatorContext ctx){
        ...
        for(int i = 0; i < ctx.outrasparcelas.size(); i++){ // Para cada termo do lado direito,
            ...
            if(parcela == Tipos.Erro) // Verifica se tem erro.
                return Tipos.Erro;
            if(opts.tiposCompativeis(parcela, ladoEsquerdo) == Tipos.Erro){ // Verifica se os tipos de cada lado são compatíveis.
                erro = true;
            } else
                ladoEsquerdo = opts.tiposCompativeis(parcela, ladoEsquerdo); // Se não houve erro, obtém se o novo tipo, por exemplo float + int = float.
        }
