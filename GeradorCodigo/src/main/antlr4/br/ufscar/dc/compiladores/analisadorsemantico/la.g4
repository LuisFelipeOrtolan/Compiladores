...
programa:
    decls=declaracoes 'algoritmo' c=corpo 'fim_algoritmo';
...
identificador:
    text=IDENT (ponto='.' text1+=IDENT)* d=dimensao;
...
corpo:
    (decls+=declaracao_local)* (lcmds+=cmd)*;
...
cmdSe:
    'se' exp=expressao 'entao' (cmds+=cmd)* (els='senao' (elsecmd+=cmd)*)? 'fim_se';

cmdCaso:
    'caso' exp=exp_aritmetica 'seja' sel=selecao (els='senao' (cmds+=cmd)*)? 'fim_caso';

cmdPara:
    'para' id=IDENT '<-' exp1=exp_aritmetica 'ate' exp2=exp_aritmetica 'faca' (cmds+=cmd)* 'fim_para';

cmdEnquanto:
    'enquanto' exp=expressao 'faca' (lcmd+=cmd)* 'fim_enquanto';

cmdFaca:
    'faca' (lcmd+=cmd)* 'ate' exp=expressao;
...
item_selecao:
    cte=constantes ':' (cmds+=cmd)*;

constantes:
    int1=numero_intervalo (',' outrosints+=numero_intervalo)*;
...
exp_aritmetica:
    termo1=termo (op+=op1 outrostermos+=termo)*;

termo:
    fator1=fator (op+=op2 outrosfatores+=fator)*;

fator:
    parcela1=parcela (op+=op3 outrasparcelas+=parcela)*;
...
parcela_nao_unario:
    comercial='&' id=identificador | cad=CADEIA;

exp_relacional:
    exp1=exp_aritmetica (oprel=op_relacional outrasexp+=exp_aritmetica)?;

op_relacional:
    igual='=' | diferente='<>' | maiori='>=' | menori='<=' | maior='>' | menor='<';

expressao:
    tl=termo_logico (op+=op_logico_1 outrostl+=termo_logico)*;

termo_logico:
    f1=fator_logico (op+=op_logico_2 outrosf+=fator_logico)*;
